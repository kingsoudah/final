...
import { calculateOverviewStats } from "../../utils/dataUtils";
...
export function OverviewTab({ data, skuCount }: OverviewTabProps) {
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Total SKUs</p>
          <p className="text-3xl font-bold text-slate-900">{skuCount.toLocaleString()}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Avg Stock/SKU</p>
          <p className="text-3xl font-bold text-slate-900">{stats.avgStock}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <p className="text-sm text-slate-500 mb-1">Categories</p>
          <p className="text-3xl font-bold text-slate-900">{stats.uniqueCategories}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200">
          <h3 className="text-lg font-semibold text-slate-800">Status Breakdown</h3>
        </div>
              ...
              {Object.entries(stats.statusCounts)
                ...
                .map(([status, count]) => (
                  <tr key={status} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4 text-sm text-center text-slate-900">{count.toLocaleString()}</td>
                    <td className="px-6 py-4 text-sm text-center">
                      <div className="flex items-center justify-center gap-2">
                        <div className="w-24 h-2 bg-slate-200 rounded-full overflow-hidden">
                          <div 
                            className="h-full bg-indigo-600"
                            style={{ width: `${(count / skuCount) * 100}%` }}
                          />
                        </div>
                        <span className="text-slate-600">{((count / skuCount) * 100).toFixed(1)}%</span>
                      </div>
                    </td>
                  </tr>
                ))}
